import { $n as Int16BufferAttribute, Cs as Uint16BufferAttribute, J as BufferGeometry, To as SRGBColorSpace, Ts as Uint8BufferAttribute, Ur as Loader, bn as Float32BufferAttribute, er as Int32BufferAttribute, q as BufferAttribute, st as Color, tr as Int8BufferAttribute, vn as FileLoader, ws as Uint32BufferAttribute } from "./three.module-C7fi3KzY.js";
	...
	*/
	parse(data) {
		...
		function getBufferAttributeClass(type) {
			switch (type) {
				case "int8":
				case "char": return Int8BufferAttribute;
				case "uint8":
				case "uchar": return Uint8BufferAttribute;
				case "int16":
				case "short": return Int16BufferAttribute;
				case "uint16":
				case "ushort": return Uint16BufferAttribute;
				case "int32":
				case "int": return Int32BufferAttribute;
				case "uint32":
				case "uint": return Uint32BufferAttribute;
				case "float32":
				case "float": return Float32BufferAttribute;
				case "float64":
				case "double": return Float64BufferAttribute;
			}
		}
		function getColorScale(type) {
			switch (type) {
				case "uchar":
				case "uint8": return 1 / 255;
				case "ushort":
				case "uint16": return 1 / 65535;
				case "float":
				case "float32":
				case "double":
		...
		function getAttributeDescriptor(properties) {
			...
			for (const customAttr of Object.keys(scope.customPropertyMapping)) {
				const matched = scope.customPropertyMapping[customAttr].map((name) => properties.find((p) => p.name === name));
				const types = matched.filter((p) => p).map((p) => p.type);
				custom[customAttr] = {
					type: types.length > 0 && types.every((type) => type === types[0]) ? types[0] : "float32",
		...
		function postProcess(buffer) {
			...
			for (const customProperty of Object.keys(scope.customPropertyMapping)) if (buffer[customProperty].length > 0) {
		...
		function handleElement(buffer, elementName, element, attributeDescriptor) {
			if (elementName === "vertex") {
				...
				if (position.usage) buffer.vertices.push(element[position.names[0]], element[position.names[1]], element[position.names[2]]);
				if (normal.usage) buffer.normals.push(element[normal.names[0]], element[normal.names[1]], element[normal.names[2]]);
				if (uv.usage) buffer.uvs.push(element[uv.names[0]], element[uv.names[1]]);
				if (color.usage) {
					const scale = getColorScale(color.type);
					const isFloat = isFloatType(color.type);
					_color.setRGB(element[color.names[0]] * scale, element[color.names[1]] * scale, element[color.names[2]] * scale, SRGBColorSpace);
